...
Use Adobe Experience Manager Headless Adaptive forms to build forms applications using front-end UI such as React and use Forms Web SDK for capabilities like state management, validation, and integrations with various other touchpoints.
...
Adobe Experience Manager Headless Adaptive forms provide such organizations freedom to build forms using their existing expertise in frontend languages while providing support to use back-end capabilities to create enterprise class forms experience.
...
1. [Create and deploy an AEM Archetype based project](#create-an-archetype-based-project)
1. [Deploy the project to AEM SDK](#deploy-the-project-to-a-local-development-environment)
1. [Create a JSON schema of Headless adaptive form and upload it to your AEM SDK instance](#create-add-json-representation-of-headless-adaptive-forms)
...
1. Run the below command:
    ...
    * Use the `includeFormsheadless=y` option to include Forms Core Components and dependencies required to include Headless Adaptive Forms functionality. On enabling this option, the following are included:  
        ...
        * A frontend React module, `ui.frontend.react.forms.af`. It helps you render a Headless Adaptive Form in a react app.  
...
1. Run the below command: 
    ...
    * Use the `includeFormsheadless=y` option to include Forms Core Components and dependencies required to include Headless Adaptive Forms functionality. On enabling this option, the following are included:  
        * The **Blank with core components** template with [core components](https://experienceleague.adobe.com/en/docs/experience-manager-core-components/using/introduction).
        * A frontend reacts module, `ui.frontend.react.forms.af`. It helps you render a Headless Adaptive Form in a react app.
...
1. Open the command prompt. If you are on Windows, open the command prompt with Administrative privileges (Run command prompt or [Git bash shell](https://khushwantsehgal.wordpress.com/2022/06/29/check-if-git-bash-is-running-in-administrator-mode/)  as an administrator).

1.  Navigate to the project directory created in the previous step. For example, `/myheadlessform`
...
1. Run the following command: 
   ...
   It can take long to resolve the dependencies and deploy the project. If there is a failure in deploying the project, see the [troubleshooting](troubleshooting.md) article for common issues and their resolution.
...
The Adaptive Form created in the previous step is blank. Configure the Adaptive Form to use the JSON schema:

1. Log in to your [AEM SDK instance](http://localhost:4502/). 

1. Navigate to Adobe Experience Manager > Forms > Forms and Documents. Select the Adaptive Form created in the previous step and tap Edit. The Adaptive Form opens in the editor. 

1. Tap the Adaptive Forms Container component and Tap Properties. It displays properties explorer in the sidebar.

1. In the properties explorer, expand the BASIC accordion, and specify the path of the JSON schema uploaded in a previous step for the Forms Runtime Document Path option. The container component displays a rendition of the form.
